...
<SCRIPT language="javascript">
	...
	var $yop = $('#TaxiPurchasing').val().split('-');
	...
	swal({
		...
		function() {
		    $.ajax({
			type: 'post',
			url: '<?php echo $this->Html->url(array('plugin' => 'taxi', 'controller' => 'taxis', 'action' => 'delete_image')); ?>',
			data: 'id=' + user_id,
			dataType: 'json',
	...
	swal({
		...
		function() {
		    $.ajax({
			type: 'post',
			url: '<?php echo $this->Html->url(array('plugin' => 'taxi', 'controller' => 'taxis', 'action' => 'delete_doc')); ?>',
			data: 'id=' + user_id,
			dataType: 'json',
			success: function(data) {
			    if (data.succ == 1) {
...
//		    options = "";
//		    $.each(subcat_data, function(index, value) {
//			options += "<label><input type='checkbox' name='data[Taxi][service_type_id][]' checked='checked' value='" + index + "' />  " + value + "</label>";
//		    });
//		    $('#sti').show();
//		    $('#sti2').html(options);
//		}
...
//	});
	$("#TaxiServiceCategoryId").on('change', function() {
	    cat_id = $(this).val();
	    $.ajax({url: "<?php echo $this->Html->url(array('plugin' => 'service_type', 'controller' => 'service_types', 'action' => 'get_service_type')); ?>",
		data: {'cat_id': cat_id},
		type: 'post',
		dataType: 'json',
		success: function(subcat_data) {
			...
			options = "";
			$.each(subcat_data, function(index, value) {
			    options += "&nbsp;&nbsp;&nbsp;<label><input type= 'checkbox' name ='data[Taxi][service_type_id][]' value='" + index + "' /> " + value + "</label>";
			});
			$('#sti ').show();
			$('#sti2').html(options);
		    }
	...
	jQuery("#TaxiEditForm").validationEngine();
...
<div id="app">
	...
	<?php echo $this->element('header'); ?>
        ...
        <div class="main-content">
            <div class="wrap-content container" id="container">
                <section id="page-title">
                    <div class="row">
                        <div class="col-sm-10">
                            <h1 class="mainTitle">Update Vehicle</h1>
                        </div>
                        <div class="col-md-2">
			    <?php
			    //echo $plate_id;die;
			    echo $this->Html->link("<i class=\"icon-arrow-left icon-white\"></i>" . __('Back To List', true) . "", array("action" => "index"), array("class" => "btn btn-green add-row", "escape" => false));
			    ?>
                        </div>
                    </div>
                </section>
		<?php echo $this->Session->flash(); ?>
		    ...
		    <?php
		    echo $this->Form->create($model, array('url' => array('plugin' => 'taxi', 'controller' => 'taxis', 'action' => 'edit'), 'enctype' => 'multipart/form-data'));
		    ...
		    ?>
					...
					?>
					    ...
					    <?php echo $this->Form->text($model . ".plate_no", array('class' => 'form-control textbox validate[required,ajax[validate_plate_edit(1)]]')); ?>
					...
					?>
					    ...
					    <?php echo $this->Form->text($model . ".cab_permit_expire", array('class' => 'form-control textbox validate[required]')); ?>
                                            <span class="help-inline" style="color: #B94A48;">
					...
					?>
						...
						<?php echo $this->Form->error($model . '.vehicle_insurance_expiry', array('wrap' => false)); ?>
                                    ...
                                    <div class="form-group <?php echo ($this->Form->error('fitness_expiry')) ? 'error' : ''; ?>">
					<?php
					...
					<div class="col-sm-12" style="border: 1px solid rgb(200, 199, 204); padding: 5px;">
						...
						<div style="" class="input col-sm-8">
						    ...
						    <?php
						    $v = isset($user["TaxiInformation"][0]['registration_certificate']) && $user["TaxiInformation"][0]['registration_certificate'] ? $user["TaxiInformation"][0]['registration_certificate'] : "";
						    if ($v) { ?>
    						    <div class="detalimg" style="display: block !important;height: 24px;line-height: 24px !important;width: 100% !important;
    							 ">
								 <?php
								 ...
								 if (is_file($file_path . $file_name)) {
								     ?>
								<a href='<?php echo WEBSITE_ADMIN_URL ?>app/download/<?php echo $file_name . "/taxidoc"; ?>'>Download Docs/Images</a>
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiInformation.permit', array('class' => "textbox validate[required]")); ?>
                                                     <?php
						    $v = isset($this->data["TaxiInformation"][0]['permit']) && $this->data["TaxiInformation"][0]['permit'] ? $this->data["TaxiInformation"][0]['permit'] : "";
						    if ($v) {
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiInformation.insurance', array('class' => "textbox validate[required]")); ?>
                                                    <?php
						    $v = isset($this->data["TaxiInformation"][0]['insurance']) && $this->data["TaxiInformation"][0]['insurance'] ? $this->data["TaxiInformation"][0]['insurance'] : "";
						    if ($v) {
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiInformation.fitness', array('class' => "textbox validate[required]")); ?>
                                                    <?php
						    $v = isset($this->data["TaxiInformation"][0]['fitness']) && $this->data["TaxiInformation"][0]['fitness'] ? $this->data["TaxiInformation"][0]['fitness'] : "";
						    if ($v) {
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiInformation.puc', array('class' => "textbox validate[required]")); ?>
                                                     <?php
						    $v = isset($this->data["TaxiInformation"][0]['puc']) && $this->data["TaxiInformation"][0]['puc'] ? $this->data["TaxiInformation"][0]['puc'] : "";
						    if ($v) {
...
<!--				    <div class="form-group <?php echo ($this->Form->error('service_category_id')) ? 'error' : ''; ?>">
				    ...
				    <?php echo $this->Form->select($model . '.service_category_id', $service_category, array("class" => "form-control validate[required]", 'empty' => "Select Motor")); ?>
                                            <span class="help-inline" style="color: #B94A48;">
...
<!--				    <div class="form-group <?php echo ($this->Form->error('service_type_id')) ? 'error' : ''; ?>" id="sti">
				    <?php
				    echo $this->Form->label($model . '.service_type_id', __('Service Type', true) . ' :<span class="required"></span>', array('style' => ""));
				    ?>
                                        <div class="multiselect <?php echo ($this->Form->error('service_type_id')) ? 'error' : ''; ?>" style="" id="sti2">
				    <?php
				    echo $this->Form->input($model . '.service_type_id', array('type' => 'select', 'multiple' => true, 'options' => $service_type, 'label' => false, 'div' => false, 'class' => 'chk-taxti', 'default' => $service_type_select, 'multiple' => true, 'multiple' => 'checkbox'));
				    ?>
                                            <span class="help-inline" style="color: #B94A48;">
				    <?php echo $this->Form->error($model . '.service_type_id', array('wrap' => false)); ?>
                                            </span>
                                        </div>
                                    </div>-->
					...
					?>
					    ...
					    <?php echo $this->Form->select($model . '.city_id', $city, array("class" => "form-control validate[required]", 'empty' => "Select City")); ?>
                                            <span class="help-inline" style="color: #B94A48;">
						<?php echo $this->Form->error($model . '.city_id', array('wrap' => false)); ?>
                                    ...
                                    <div class="col-sm-12" style="border: 1px solid rgb(200, 199, 204);">
                                        <div class="col-sm-12">
					    <?php
					    echo $this->Form->label($model . '.heating', __('Heating', true) . ' :<span class="required"></span>', array('class' => "col-sm-4"));
					    ?>
						...
						<?php echo $this->Form->Checkbox($model . '.heating', array('empty' => false)); ?>Yes
						    ...
						    <?php echo $this->Form->error($model . '.ac', array('wrap' => false)); ?>
                                        ...
                                        <div class="col-sm-12">
					    <?php
					    echo $this->Form->label($model . '.music_system', __('Music system', true) . ' :<span class="required"></span>', array('class' => "col-sm-4"));
					    ?>
                                            <div class="input col-sm-8" style="" >
						...
						?>
                                        ...
                                        <div class="col-sm-12">
					    <?php
					    echo $this->Form->label($model . '.anti_thief_alarm', __('GPS Device', true) . ' :<span class="required"></span>', array('class' => "col-sm-4"));
					    ?>
                                            <div class="input col-sm-8" style="" >
						...
						?>
                                        ...
                                        <div class="col-sm-12">
					    <?php
					    echo $this->Form->label($model . '.Roof Carrier', __('Roof Carrier', true) . ' :<span class="required"></span>', array('class' => "col-sm-4"));
					    ?>
                                            <div class="input col-sm-8" style="" >
				    ...
				    <div class="clr clear"></div>
				    <div class="form-group">
					<label style="">Upload Photos :<span class="symbol required"></span></label>
					<div class="col-sm-12" style="border: 1px solid rgb(200, 199, 204); padding: 5px;">
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.front', array('class' => "textbox validate[required]")); ?>

						    <?php
						    //pr($user['TaxiImage']);
						    $v = isset($user['TaxiImage'][0]["front"]) && $this->data['TaxiImage'][0]["front"] ? $this->data['TaxiImage'][0]["front"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;

						    $image_url = $this->Html->url(array('plugin' => 'imageresize', 'controller' => 'imageresize', 'action' => 'get_image', 50, 50, base64_encode($file_path), $file_name), true);
						    if (is_file($file_path . $file_name)) {
							echo $images = $this->Html->image($image_url, array('alt' => $this->data['Taxi']['plate_no'], 'title' => $this->data['Taxi']['plate_no']));
						    } else {
							echo $this->Html->image('no_image.jpg', array('width' => '50px', 'height' => '50px'));
						    }
						    ?>
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.rear', array('class' => "textbox validate[required]")); ?>
						    <?php
						    $v = isset($this->data['TaxiImage'][0]["rear"]) && $this->data['TaxiImage'][0]["rear"] ? $this->data['TaxiImage'][0]["rear"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.left_side', array('class' => "textbox validate[required]")); ?>
						    <?php
						    $v = isset($this->data['TaxiImage'][0]["left_side"]) && $this->data['TaxiImage'][0]["left_side"] ? $this->data['TaxiImage'][0]["left_side"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.right_side', array('class' => "textbox validate[required]")); ?>
						    <?php
						    $v = isset($this->data['TaxiImage'][0]["right_side"]) && $this->data['TaxiImage'][0]["right_side"] ? $this->data['TaxiImage'][0]["right_side"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.inside_from_front', array('class' => "textbox validate[required]")); ?>
						    <?php
						    $v = isset($this->data['TaxiImage'][0]["inside_from_front"]) && $this->data['TaxiImage'][0]["inside_from_front"] ? $this->data['TaxiImage'][0]["inside_from_front"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;
						...
						<div style="" class="input col-sm-8">
						    <?php echo $this->Form->file('TaxiImage.inside_from_rear', array('class' => "textbox validate[required]")); ?>
						    <?php
						    $v = isset($this->data['TaxiImage'][0]["inside_from_rear"]) && $this->data['TaxiImage'][0]["inside_from_rear"] ? $this->data['TaxiImage'][0]["inside_from_rear"] : "";
						    $file_path = TAXI_IMAGE_STORE_PATH;
						    $file_name = $v;
					...
					?>
					    ...
					    <?php echo $this->Form->select($model . '.motor_type_id', $motor_type, array("class" => "form-control validate[required]", 'empty' => "Select Vehicle Category")); ?>
                                            <span class="help-inline" style="color: #B94A48;">
						<?php echo $this->Form->error($model . '.motor_type_id', array('wrap' => false)); ?>
                                            </span>
                                            <img src="<?php echo WEBSITE_URL; ?>img/ajax.gif" id="motor_cat_img" style="display:none;"  />
                                        </div>
                                    </div>

                                    <div class="form-group <?php echo ($this->Form->error('motor_model_id')) ? 'error' : ''; ?>">
					<?php
					echo $this->Form->label($model . '.motor_model_id', __('Select Model', true) . ' :<span class="required"></span>', array('style' => ""));
					?>
                                        <div class="input <?php echo ($this->Form->error('motor_model_id')) ? 'error' : ''; ?>" style="" >
					    <?php echo $this->Form->select($model . '.motor_model_id', $motor_model, array("class" => "form-control validate[required]", 'empty' => "Select Motor")); ?>
                                            <span class="help-inline" style="color: #B94A48;">
					...
					<?php
					echo $this->Form->label($model . '.motor_id', __('Select Vehicle', true) . ' :<span class="required"></span>', array('style' => ""));
					?>
                                        <div class="input <?php echo ($this->Form->error('motor_id')) ? 'error' : ''; ?>" style="" >
					    <?php echo $this->Form->select($model . '.motor_id', $motor, array("class" => "form-control validate[required]", 'empty' => "Select Vehicle")); ?>
                                            <span class="help-inline" style="color: #B94A48;">
				    ...
				    <?php echo $this->Html->link(__('Cancel <i class="fa fa-arrow-circle-right"></i>', true), array("action" => "index"), array("class" => "btn btn-primary btn-wide pull-right", "escape" => false)); ?>
                                </div>
                            </div>
...
<script>
	...
	if (id < 6)
	{
	    /*var html = '<div id="imageremove'+id+'" class="ad_img" style="position:relative; margin-top:15px;"><a href="javascript:;" class=""><input type="file" class="validate[required,checkFileType["png"]]" id="DealImageImage'+id+'" onchange="$(&quot;#upload-file-info'+id+'&quot;).html($(this).val());" class="form-control textbox browsers_button" name="data[TaxiImage][image]['+id+']"><span style="color: #B94A48;" class="help-inline"></span></a></span><a href="javascript:void()" class="text-danger" id="remove'+id+'"><i class=" glyphicon glyphicon-remove-circle"></i> Remove Image</a></div>';*/
