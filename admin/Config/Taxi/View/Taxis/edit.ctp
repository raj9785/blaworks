...
// Highlight checkboxes that the user selects
    ...
    $(function () {
        ...
        $('#TaxiOwnerId').on('change', function () {
            ...
            get_drivers(vendor_id, cur_id);
        });
        $('#TaxiCompanyId').on('change', function () {
            var vendor_id = $(this).val();
            var owner_type = $('#TaxiOwnerId').val();

            get_drivers(vendor_id, owner_type);
        });
        get_drivers($('#TaxiCompanyId').val(), $('#TaxiOwnerId').val());
    });

    function get_drivers(vendor_id, owner_type) {
        if (owner_type == 2) {
            $('#driver_owner').show();
            if (vendor_id == '') {
        ...
        var selected = $('#driverownerid').val();
        $.ajax({
	...
	<?php echo $this->element('header'); ?>
        ...
        <div class="main-content">
            <div class="wrap-content container" id="container">
			    ...
			    //echo $plate_id;die;
			    if($vendorid_return){
				echo $this->Html->link("<i class=\"icon-arrow-left icon-white\"></i>" . __('Back To List', true) . "", array('plugin' => 'motor_model', 'controller' => 'motor_models', 'action' => 'vehiclelist', $vendorid_return), array("class" => "btn btn-green add-row", "escape" => false));
			    }else{
		...
		<?php echo $this->Session->flash(); ?>
		  ...
		  //  echo $this->Form->create($model, array('url' => array('plugin' => 'taxi', 'controller' => 'taxis', 'action' => 'edit'), 'enctype' => 'multipart/form-data'));
			...
			echo $this->Form->create($model, array('url' => array('plugin' => 'taxi', 'controller' => 'taxis', 'action' => 'edit', $id, "?" => $query_data), 'enctype' => 'multipart/form-data'));
               
			
		    echo $this->Form->hidden('id');
		    ?>
                    <input type="hidden" name="vendorid_return" value="<?php echo $vendorid_return ?>">
				...
				?>
				    ...
				    <?php echo $this->Form->select($model . '.owner_id', array(1=>'Vendor Cum Owner',2=>'Driver Cum Owner',3=>'Other'), array("class" => "form-control validate[required]", 'empty' => "Select Ownership Type")); ?>
                                    <span class="help-inline" style="color: #B94A48;">
					<?php echo $this->Form->error($model . '.owner_id', array('wrap' => false)); ?>
                        ...
                        <input type="hidden" name="data[Taxi][driverownerid]" id="driverownerid" value="<?php echo isset($driver_id)?$driver_id:'';?>">
                        ...
                        <div class="col-md-6">
                            <div class="form-group <?php echo ($this->Form->error('motor_model_id')) ? 'error' : ''; ?>">
                                ...
                                <div class="input <?php echo ($this->Form->error('motor_model_id')) ? 'error' : ''; ?>" style="" >
										<?php echo $this->Form->select($model . '.motor_model_id', $motor_model, array("class" => "form-control validate[required]", 'empty' => "Select")); ?>
                                    <span class="help-inline" style="color: #B94A48;">
