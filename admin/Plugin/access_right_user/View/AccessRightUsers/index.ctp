...
<script>
    $(document).ready(function () {
       /*  $('#AccessRightUserFromDate').datepicker({
            dateFormat: "yy-mm-dd",
            changeMonth: true,
            changeYear: true,
            onClose: function (selectedDate) {
                $("#AccessRightUserToDate").datepicker("option", "minDate", selectedDate);
            }
        });
        $('#AccessRightUserToDate').datepicker({
            dateFormat: "yy-mm-dd",
            changeMonth: true,
            changeYear: true,
            onClose: function (selectedDate) {
                $("#AccessRightUserFromDate").datepicker("option", "maxDate", selectedDate);
            }
		...
		$('#AccessRightUserFromDate').datepicker({
			...
			onSelect: function (selectedDate) {
				...
				$('#AccessRightUserToDate').datepicker("option", {
...
<script type='text/javascript'>
    $(function () {
        ...
        $("#AccessRightUserMyAction").on('change', function () {
            value = $(this).val();
            if (value == 'active') {
                $("#active").click();
            }
        ...
        swal({
            ...
            closeOnConfirm: false,
                ...
                function () {
                    $.ajax({
                        type: 'post',
                        url: '<?php echo $this->Html->url(array('plugin' => 'access_right_user', 'controller' => 'access_right_users', 'action' => 'deleterow')); ?>',
                        data: 'id=' + user_id,
                        dataType: 'json',
        ...
        swal({
            ...
            closeOnConfirm: false,
                ...
                function () {
                    $.ajax({
                        type: 'post',
                        url: '<?php echo $this->Html->url(array('plugin' => 'access_right_user', 'controller' => 'access_right_users', 'action' => 'change_status_active')); ?>',
                        data: 'id=' + user_id,
                        dataType: 'json',
                        success: function (data) {
                            if (data.succ == 1) {
        ...
        swal({
            ...
            closeOnConfirm: false,
                ...
                function () {
                    $.ajax({
                        type: 'post',
                        url: '<?php echo $this->Html->url(array('plugin' => 'access_right_user', 'controller' => 'access_right_users', 'action' => 'change_status_inactive')); ?>',
                        data: 'id=' + user_id,
                        dataType: 'json',
                        success: function (data) {
                            if (data.succ == 1) {
            ...
            <div class="wrap-content container" id="container">
                <section id="page-title">
                    <div class="row">
                        ...
                        <div class="col-sm-2">						
						<?php 			
							echo $this->Html->link('<i class="fa fa-plus"></i> ' . __('Add New User', true) . "", array('plugin' => "access_right_user", 'controller' => 'access_right_users', "action" => "add"), array("class" => "btn btn-green add-row", "escape" => false)); 				
						?>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="row">
                        <div class="col-md-12 space20">
                            <div class="pull-left1 driver_index1">
								...
								echo $this->Form->select('access_right_category_id', $AccessRightCategory, array('value'=>isset($category_id) && $category_id?$category_id:'', 'empty' => __('Category Name', true), 'class' => 'form-control input-sm')) . '&nbsp;&nbsp;';
								...
								echo $this->Form->text('firstname', array('placeholder' => __('Employee Name', true), 'value'=>isset($firstname)?$firstname:'' ,'class' => ' form-control input-sm')) . '&nbsp;';
                                ...
                                &nbsp;<a class="btn btn-green add-row new-style " href="<?php echo $this->Html->url(array('plugin' => 'access_right_user', 'controller' => 'access_right_users', 'action' => 'index')); ?>">Reset</a>
                    ...
                    <table class="table table-striped table-bordered  table-full-width" id="<?php echo (!empty($result)) ? 'id="sample_1"' : '' ?>">
                        <thead>
                            ...
                            <tr style="height:30px;">
                                ...
                                <th class="hidden-xs"><?php echo __('Category Name'); ?></th>
                                <th class="hidden-xs">User Name</th>

                                <th class="hidden-xs"><?php echo __('Employee ID'); ?></th>
                                <th class="hidden-xs"><?php echo __('Employee Name'); ?></th>
				...
				foreach ($result as $records) {
				    ?>
                            <tr style="height:30px;" class="gallerytr">
                                ...
                                <td><?php echo $records['AccessRightUser']['username']; ?></td>
                                <td><?php echo $records['AccessRightUser']['employee_id']; ?></td>
                                <td><?php echo $records['AccessRightUser']['firstname']; ?></td>
                                <td><?php echo $records['AccessRightUser']['email']; ?></td>
                                <td><?php echo $records['AccessRightUser']['mobile']; ?></td>
						 ...
						 src="<?php echo WEBSITE_URL . "img/" . $status_img; ?>" width="20" height="20"/>
						    ...
						    <?php echo __('Action'); ?> <span class="caret"></span>
                                        ...
                                        <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
                                            <li>
									<?php
											echo $this->Html->link('Edit' . __('', true), array('plugin' => 'access_right_user', 'controller' => 'access_right_users', 'action' => 'edit',$records[$model]['id']), array('class' => '', 'escape' => false));       
									?>							
                                            </li>
                                            <li>
